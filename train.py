...
def train(
    ...
    for epoch in range(epochs):
        ...
        for idx, (tokens, mask, prefix) in enumerate(train_dataloader):
            ...
            progress.set_postfix({"avg_loss": running_loss / (idx + 1)})
            progress.update()

        del tokens, mask, prefix, outputs, logits
        torch.cuda.empty_cache()

        step = epochs * len(train_dataloader) + idx
        avg_loss = running_loss / (idx + 1)
        mlflow.log_metric("train_loss", avg_loss, step)
        progress.close()

        print(f">>> Validation epoch {epoch}")
        sys.stdout.flush()
        vprogress = tqdm(total=len(val_dataloader), desc=output_prefix)
        with torch.no_grad():
            for vidx, (tokens, mask, prefix) in enumerate(val_dataloader):
                tokens, mask, prefix = (
                    tokens.to(DEVICE),
                    mask.to(DEVICE),
                    prefix.to(DEVICE, dtype=torch.float32),
                ...
                vloss = nnf.cross_entropy(
                    logits.reshape(-1, logits.shape[-1]),
                    tokens.flatten(),
                    ignore_index=0,
                )
                running_vloss += vloss.item()
                vprogress.set_postfix({"avg_val_loss": running_vloss / (vidx + 1)})
                vprogress.update()

        del tokens, mask, prefix, outputs, logits
        torch.cuda.empty_cache()

        vstep = epochs * len(val_dataloader) + vidx
        avg_vloss = running_vloss / (vidx + 1)
        mlflow.log_metric("val_loss", avg_vloss, vstep)
        vprogress.close()

        print(f"... Avg. Training Loss: {avg_loss}")
        print(f"... Avg. Validation Loss: {avg_vloss}")
    ...
    mlflow.set_tag("Best Epoch", best_epoch)
...
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--train_data", default="./deepfashion-mm/deepfashion_ViT-B_32_train.pkl"
    )
    parser.add_argument(
        "--val_data", default="./deepfashion-mm/deepfashion_ViT-B_32_val.pkl"
    )
    parser.add_argument("--out_dir", default="/checkpoints")
    parser.add_argument(
        "--prefix", default="deepfashion_clipcap", help="prefix for saved filenames"
    )
    ...
    parser.add_argument("--mlflow_experiment", type=str, default="clipcap_deepfashion")
    args = parser.parse_args()
    timestr = time.strftime("%Y%m%d_%H%M%S")
    output_dir = os.getcwd() + args.out_dir + f"/clipcap_{timestr}"
    prefix_length = args.prefix_length

    print("Training", end=" ")
    train_dataset = ClipDataset(
        args.train_data, prefix_length, normalize_prefix=args.normalize_prefix
    )

    print("Validation", end=" ")
    val_dataset = ClipDataset(
        args.val_data, prefix_length, normalize_prefix=args.normalize_prefix
    )
    ...
    with mlflow.start_run() as run:
        ...
        if args.only_prefix:
            ...
            print("Train mode: both prefix and GPT\n")
            sys.stdout.flush()
        train(
            train_dataset,
            val_dataset,
